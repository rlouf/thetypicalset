...
I think that we cannot do better than something descriptive with a hierarchical model here. Which, in itself, is not very interesting. But we should nevertheless be able to reproduce the original distirbutions. We could then look for variables that explain the different transitions (or, first, the state of affairs)
