...
aliases:
  - 2e41e200-be7a-482b-8cfe-d0d67df26920
---
...
- [Parallel Random Numbers: As easy as 1, 2, 3](http://www.thesalmons.org/john/random123/papers/random123sc11.pdf) (2011)

On splittable PRNG design for functional programs:

- [Distributed Random Number Generation](https://www.cambridge.org/core/journals/journal-of-functional-programming/article/distributed-random-number-generation/6D10F1D0A2FB7E66D5F746F6D0822D78) (2008)
- [Splittable PRNGs Using Cryptographic Hashing](https://publications.lib.chalmers.se/records/fulltext/183348/local_183348.pdf) (2013)
...
- In [\#1036](https://github.com/aesara-devs/aesara/pull/1036) the use of `default_output` to hide the PRNG state from the user is causing multiple headaches in the etuplization of `RandomVariable=\s and unification/reification of expressions with =RandomVariable=\s. This is the only =Op` in Aesara that makes use of this property, and to "special-casing" the etuplization logic for =RandomVariable=`\s`{=latex} often appeared as the easiest solution.

- In [\#66](https://github.com/aesara-devs/aemcmc/pull/66#issuecomment-1258471312) in AeMCMC, expressing expansions like the following convolution of two normal variables is overly complex:
...
In other words, =RandomVariable=`\s`{=latex} are responsible for **both** advancing the state of the PRNG, and producing a random value. This double responsibility is what creates graph dependencies between nodes that have otherwise no data dependency. The following snippet illustrates this:
...
- The fact that callers cannot be intentional about what they do with the random state is limiting. This can be necessary in pratical applications, for instance to implement [coupled sampling algorithms](https://statisfaction.wordpress.com/2017/09/17/unbiased-hamiltonian-monte-carlo-with-couplings/) in which two algorithms share the same random state.
...
1.  `RandState` and `split` implementation
...
1.  Numba
    ...
    After [\#1245](https://github.com/aesara-devs/aesara/pull/1245) Aesara will support NumPy's Generator API. Furthermore NumPy has support for [Philox as a BitGenerator](https://numpy.org/doc/stable/reference/random/bit_generators/philox.html), a [counter-based PRNG](http://www.thesalmons.org/john/random123/papers/random123sc11.pdf) which can easily accomodate [splittable PRNG representations](https://publications.lib.chalmers.se/records/fulltext/183348/local_183348.pdf). Assuming we can map each path in the PRNG graph to a `(key, counter)` tuple, the transpilation of `RandomStream=\s using the Philox =BitGenerator` should be straighforward. For the explicit splitting interface, we can directly translate the =RandomVariable=`\s`{=latex} to NumPy =Generator=`\s`{=latex} and seed these generators at compile time. So that:
...
2.  JAX
    ...
    Transpilation to JAX would be straightforward, as JAX [uses a splittable PRNG representation](https://jax.readthedocs.io/en/latest/jep/263-prng.html). We will simply need to perform the following substitutions:
...
With the abstractions that are currently available in Aesara/AePPL [one wants to define the truncation operator](https://github.com/aesara-devs/aeppl/pull/131) as:
...
This is somewhat similar to what is done in the [Anglican](https://probprog.github.io/anglican/) probabilistic programming language implemented in Clojure. We retain the existing ambiguity that exists between random variables and their realizations, but that does not matter mathematically speaking.
...
Or maybe now more simply using the [[mixture_of_probability_measures|measure-theoretic definition of mixtures]] as:
...
This is slightly awkard before `logdensity` applies to `Measure=\s while =joint_logdensity` works with =RandomVariable=`\s`{=latex}. I am not sure if we want to resolve this ambiguity by using a different name that might be confusing to the user.
...
See [Eric Ma's blog post on the topic](https://ericmjl.github.io/blog/2019/5/29/reasoning-about-shapes-and-probability-distributions/).
...
1.  Monkey patching to specialize the RV =Op=`\s`{=latex}
