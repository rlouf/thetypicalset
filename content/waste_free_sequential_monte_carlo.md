...
1.  sampled~idx~ <- resample(M, weights)
2.  sampled~particles~ = particles[sampled~idx~]
3.  traces = jax.scan(jax.vmap(mcmc)(keys, new~particles~), P)
...
1.  sampled~particles~ = particles
2.  traces = jax.scan(jax.vmap(mcmc)(keys, new~particles~), P)
