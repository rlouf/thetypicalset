...
The [Zebra Puzzle](https://en.wikipedia.org/wiki/Zebra_Puzzle), originally published in *Life International* in 1962, is a must-have example for logic programming languages. [This repository](https://github.com/MNoorFawi/logic-programming-in-python) proposes a solution `logpy`, `kanren`'s ancestor, but [does not seem to be working](https://github.com/pythological/kanren/issues/51) with `kanren`. In this blog post I will propose a [kanren](https://github.com/pythological/kanren) solution, and show how we can make the solution much more readable than other languages' using dataclasses.
...
In a next post we will see how you can make your own objects "unifiable" and thus use relational programming with an existing codebase. This is already what [is happening in aesara](https://github.com/aesara-devs/aesara/blob/main/aesara/graph/unify.py), where Ops and Variables have been made unifiable so we can do some [[~/projects/thetypicalset/org/blog/identify-horsehoe|pattern matching on Aesara graphs]].
